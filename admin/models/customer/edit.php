...
if(isset($_GET['customer_id'])){
    ...
    $sql = "SELECT id,surname,name,email,phone,birthday,access,sex FROM users WHERE id=?";
    $user = query($sql,[$customer_id])->fetch(PDO::FETCH_ASSOC);
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($errors)){
        $sql = "UPDATE users SET surname=?,name=?,email=?,phone=?,birthday=?,sex=? WHERE id=?";
        $result = query($sql,[
            $surname,
            $name,
            $email,
